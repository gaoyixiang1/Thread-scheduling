...
int main() {
    ...
    pthread_t threads[num_threads + 1]; // 主要任务线程 + 干扰线程
    
    printf("Starting main...\n");
    
    // 创建主要任务线程
    pthread_create(&threads[0], NULL, my_task_func, NULL);

    // 创建干扰线程
    for (int i = 1; i <= num_threads; i++) {
        pthread_create(&threads[i], NULL, my_task_func, NULL);
    }
